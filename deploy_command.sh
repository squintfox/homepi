...
while [[ $# -gt 0 ]]; do
       case "$1" in
	       ...
	       --local)
		       ...
		       ;;
	       -h|--help)
		       echo "Usage: $0 [--no-build] [--no-deploy] [--force] [--local]"
		       exit 0
		       ;;
	       *)
		       echo "Unknown argument: $1" >&2
		       echo "Usage: $0 [--no-build] [--no-deploy] [--force] [--local]" >&2
		       exit 1
		       ;;
       esac
done

# build stack image(s) and push to local registry
if [[ "$DO_BUILD" == "true" ]]; then
	BUILD_COMPOSE_FILES=(-f "$STACK_NAME"/docker-compose.build.yml)
	if [[ "$USE_LOCAL" == "true" ]]; then
		BUILD_COMPOSE_FILES+=(-f "$STACK_NAME"/docker-compose.build-local.yml)
	fi
	docker compose "${BUILD_COMPOSE_FILES[@]}" build --no-cache

	echo "Pushing docker image to local registry..."
	if ! docker compose "${BUILD_COMPOSE_FILES[@]}" push; then
...
if [[ "$DO_DEPLOY" == "true" ]]; then
	...
	if [[ "$USE_LOCAL" == "true" ]]; then
		COMPOSE_FILES+=(-c "$STACK_NAME"/docker-compose.local.yml)
	fi
